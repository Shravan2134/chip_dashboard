...
**Complete Documentation: How Everything Works, All Formulas, Display Rules**
...
3. [Partial Payment Logic (Complete Flow)](#partial-payment-logic)
4. [Pending Payments Calculation](#pending-payments-calculation)
5. [Share Calculations](#share-calculations)
6. [How Values Are Displayed in UI](#how-values-are-displayed)
...
def settle_payment(client_exchange_id, payment, balance_record_id):
    ...
    # STEP 20: Enforce invariants (INSIDE transaction, BEFORE commit)
    try:
...
```python
def get_pending_for_my_client(client_exchange):
    """Get pending for MY CLIENTS"""
    ...
    return {
        ...
        'old_balance': capital,
        'current_balance': cb,  # ✅ From loss snapshot
        'total_loss': loss
    }
```
...
CAPITAL = Σ(FUNDING) − Σ(CAPITAL_CLOSED) (from ledger)
CB = loss_snapshot.balance_record.remaining_balance (causal)
ClientPayable = (LOSS × my_share_pct) / 100
...
```python
def get_pending_for_company_client(client_exchange):
    """Get pending for COMPANY CLIENTS"""
    # Step 1: Get active loss snapshot
    active_loss = LossSnapshot.objects.filter(
        client_exchange=client_exchange,
        is_settled=False
...
- Only when `balance_record.profit_consumed = False`
...
CAPITAL_new = 70.00
CB = 40.00 (unchanged, from snapshot)
...
✅ PROFIT withdrawal is atomic (profit_consumed flag)
```
...
✅ Snapshot-Ledger consistency enforced (invariant)
```
