# 📘 Pending Payments: Complete Formulas & Logic Guide
...
🚨 CRITICAL: NET is informational only.
...
FULL Settlement (when CAPITAL_new <= CB + epsilon):
    CAPITAL_new = CB
    (Uses epsilon check, not exact equality - prevents floating-point bugs)
```
...
On PROFIT_WITHDRAWAL:
    ...
    ASSERT CB_new >= CAPITAL  (prevent fake loss)
```
